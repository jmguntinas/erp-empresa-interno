...
/* ====== Tabla objetivo (companies) ====== */
$TABLE = td_get_table($pdo, 'companies');
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (($_POST['action']??'')==='save_columns')) {
  ...
  $_SESSION['firm_visible'][$TABLE]=$vis;

  $sea=[]; foreach($allCols as $c) $sea[$c] = isset($_POST['search__'.$c])?1:0;
  $_SESSION['firm_searchable'][$TABLE]=$sea;

  $_SESSION['firm_per'][$TABLE] = max(10, min(200, (int)($_POST['cfg_per'] ?? 25)));
  header('Location: '.basename($_SERVER['PHP_SELF'])); exit;
}
$visible = $_SESSION['firm_visible'][$TABLE] ?? null;
$searchables = $_SESSION['firm_searchable'][$TABLE] ?? null;
$cfgPerPage = (int)($_SESSION['firm_per'][$TABLE] ?? 25);
...
<h4 class="mb-3 d-flex align-items-center gap-2">
  <i class="bi bi-buildings"></i> <?= ti($pdo, 'menu.empresas', 'Empresas') ?>
  <span class="ms-auto d-flex gap-2">
    <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#configModal"
            data-bs-toggle="tooltip" title="<?= h(ti($pdo,'btn.configuracion','Configuración')) ?>">
